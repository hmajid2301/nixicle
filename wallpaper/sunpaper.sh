...
#
wallpaperPath="$HOME/wallpaper/LakesideDeer"
...
setpaper_construct() {
	...
	if [ "$swww_enable" == "true" ]; then
		...
		gsettings set org.gnome.desktop.background picture-uri-dark "$wallpaperPath"/"$image".jpg
...
get_moonphase() {
	...
	if [[ $phase_number -ge 0 ]] && [[ $phase_number -lt 1000 ]]; then
		phase_addendum="-1" # new
	elif [[ $phase_number -ge 1000 ]] && [[ $phase_number -lt 6382 ]]; then
		phase_addendum="-2" # waxing crescent
	elif [[ $phase_number -ge 6382 ]] && [[ $phase_number -lt 8382 ]]; then
		phase_addendum="-3" # first quarter
	elif [[ $phase_number -ge 8382 ]] && [[ $phase_number -lt 13765 ]]; then
		phase_addendum="-4" # waxing gibbous
	elif [[ $phase_number -ge 13765 ]] && [[ $phase_number -lt 15765 ]]; then
		phase_addendum="-5" # full
	elif [[ $phase_number -ge 15765 ]] && [[ $phase_number -lt 21147 ]]; then
		phase_addendum="-6" # waning gibbous
	elif [[ $phase_number -ge 21147 ]] && [[ $phase_number -lt 23147 ]]; then
		phase_addendum="-7" # last quarter
	elif [[ $phase_number -ge 23147 ]] && [[ $phase_number -lt 28530 ]]; then
		phase_addendum="-8" # waning crescent
	else
		phase_addendum="-1" # back to new
	fi
}
