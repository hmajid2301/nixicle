    ...
    nixos = { config, lib, ... }: {
      services = {
        ...
        postgresql = {
          ...
          ensureUsers = [{ name = "grafana"; ensureDBOwnership = true; }];
        };

        loki = {
          enable = true;
          configuration = {
            server = { http_listen_port = 3030; grpc_listen_port = 3031; };
            auth_enabled = false;
            ingester = {
              lifecycler = {
                address = "127.0.0.1";
        ...
        tempo = {
          enable = true;
          settings = {
            server = { http_listen_port = 4400; grpc_listen_port = 4401; };
        ...
        traefik.dynamicConfigOptions.http = lib.mkMerge [
          (lib.nixicle.mkTraefikService { name = "prometheus"; port = 3020; })
          (lib.nixicle.mkTraefikService { name = "grafana"; port = 3010; })
          (lib.nixicle.mkTraefikService { name = "alertmanager"; port = 9093; })
