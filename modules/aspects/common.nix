# common aspect — replaces roles.common.enable = true on both NixOS and home-manager.
# Included by the desktop aspect (and any server aspect that wants the base).
{ den, ... }:
{
  den.aspects.common = {
    # NixOS side: networking, ssh, sops, yubikey, nix config, boot, locale, stylix
    nixos = { lib, ... }: {
    ...
    # Home-manager side: browsers, terminals, shells, tools, stylix
    homeManager = { ... }: {
