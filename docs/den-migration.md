...
- [x] `modules/aspects/performance.nix` (Tiered: base / responsive / max)
- [x] `modules/aspects/nfs-truenas.nix` (DRY)
...
- [x] nixCats replaced by `nix-wrapper-modules` (see `docs/neovim-wrapper-migration.md`)
- [x] All neovim plugin inputs (`plugins-*`) declared via `flake-file.inputs` in `modules/aspects/neovim/default.nix`
- [x] opencode, nix-playwright-mcp, zellij-mcp inputs moved into aspect files
- [x] nixflix input moved into `modules/aspects/services/nixflix.nix`
- [x] comma input already in `flake.nix` (not yet moved to flake-file)
- [ ] Add remaining inputs via flake-file: hyprland, comma, fenix, omerxx-dotfiles
...
### Phase 7 — Housekeeping (post-migration)
- [x] `homeConfigurations."haseeb@workstation"` exposed for standalone `nh home switch`
- [x] `homeConfigurations."haseebmajid@dell"` standalone home working
- [x] `haseeb` added to `wheel` group on workstation, framework, framebox (fixes trusted-user nix warnings)
- [x] `polkit-gnome-authentication-agent-1` systemd service removed from niri aspect (niri-flake provides its own)
- [x] `programs.git.settings` URL subsection fixed (`url."git@github.com:"` nested attrset generates correct `[url "git@github.com:"]` space-syntax for libgit2)
- [x] opencode theme moved from deprecated `settings.theme` to `tui.theme` (opencode v1.2.15+ requires separate `tui.json`)
- [x] lanzaboote `autoEnrollKeys.autoReboot = false` (prevents PCR churn breaking TPM2 LUKS on reboot)

### Final Cleanup
- [x] Delete `modules/legacy.nix` (already gone)
- [x] Delete `old/modules/nixos/` and `old/modules/home/` trees (already gone; `old/` only has images, infra, tasks)
