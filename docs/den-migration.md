...
**Goal:** The `flake.nix` is auto-generated by `flake-file`[^12]. Input
declarations move into the module files that need them — total colocation.

**Create input declarations in aspect files:**

```nix
# modules/aspects/impermanence.nix — input lives with the aspect
{ den, inputs, ... }: {
  flake-file.inputs.impermanence.url = "github:nix-community/impermanence";

  den.aspects.impermanence = {
    nixos.imports = [ inputs.impermanence.nixosModules.default ];
    nixos.environment.persistence."/persist".allowTrash = true;
    homeManager.home.persistence."/persist".hideMounts = false;
  };
}

# modules/aspects/roles/gaming.nix — nix-gaming input lives here
{ den, inputs, ... }: {
  flake-file.inputs.nix-gaming.url = "github:fufexan/nix-gaming";

  den.aspects.gaming.provides.performance.nixos = { ... }: {
    imports = [
      inputs.nix-gaming.nixosModules.platformOptimizations
      inputs.nix-gaming.nixosModules.pipewireLowLatency
    ];
...
The generated `flake.nix` is a single expression — never edit it manually.
